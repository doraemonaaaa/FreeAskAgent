...
def create_llm_engine(model_string: str, use_cache: bool = False, is_multimodal: bool = False, **kwargs) -> Any:
    ...
    if os.getenv("Proxy_API_BASE") is not None:
        print("Use Proxy URL to call API")
        is_proxy = True
    if is_proxy == True:
        from .proxy import ProxyAI
        config = {
    ...
    # === Local Qwen3-VL via Transformers ===
        ...
        config = {
            ...
            "use_cache": use_cache,
            "is_multimodal": is_multimodal,
            # Left as None so LocalQwen3VL applies its own defaults (bf16, and
            # a single GPU that fits) instead of transformers' "auto", which
            # honours the checkpoint's fp32 config and shards across busy GPUs.
