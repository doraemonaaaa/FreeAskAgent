...
class Planner:
    ...
    def summarize_input_data(self, items):
        ...
        for i, item in enumerate(items):
            if isinstance(item, (bytes, bytearray)):
                summary.append({"index": i, "type": "bytes", "length": len(item)})
            else:
                s = str(item)
                summary.append(
                    {
    ...
    def extract_context_subgoal_and_tool(self, response: Any) -> Tuple[str, str, str]:
        def normalize_tool_name(tool_name: str) -> str:
            """Normalize tool name robustly (spaces/underscores)."""
    ...
    def analyze_query(self, question: str, image_paths: Optional[Union[str, Sequence[str]]] = None) -> str:
        image_info = get_image_info(image_paths)

        query_prompt = QuerynalysisPrompt(self.available_tools, self.toolbox_metadata, question, image_info)
        input_data: List[Any] = [query_prompt]
    ...
    def generate_base_response(self, question: str, image_paths: Optional[Union[str, Sequence[str]]] = None, max_tokens: int = 4000) -> str:
        """Optional: plain LLM response without tools/memory."""
        image_info = get_image_info(image_paths)
        prompt = f"""You are a helpful assistant.
Query: {question}
Image: {image_info}
"""
        input_data: List[Any] = [prompt]
        if self.is_multimodal:
            for path in normalize_image_paths(image_paths):
                try:
                    ...
                    pass
        return self.llm_engine(input_data)

    def _build_answer_prompt(self, question: str, image_paths: Optional[Union[str, Sequence[str]]], memory: Memory, detailed: bool) -> List[Any]:
        """Build a prompt list (text + optional image bytes) for answer generation."""
        image_info = get_image_info(image_paths)

        retrieved = ""
        try:
            retrieved = memory.get_retrieved_context(question, k=5)
        except Exception:
            retrieved = ""

        style = "Detailed solution with reasoning steps and key considerations." if detailed else "Give the final answer directly and succinctly."

        prompt = f"""Context:
Query: {question}
Image: {image_info}

Retrieved Memory (A-Mem):
{retrieved}

Actions Taken (Tool Traces):
{memory.get_actions()}
...
"""
        ...
        if self.is_multimodal:
            ...
            if len(paths) > 1:
                ...
                input_data.append(f"Consider the following {len(paths)} frames in chronological order: {filenames}.")
            for path in paths:
                if not os.path.isfile(path):
                    print(f"Warning: image file not found '{path}' - skipping.")
                    continue
    ...
    def generate_final_output(self, question: str, image_paths: Optional[Union[str, Sequence[str]]], memory: Memory) -> str:
        input_data = self._build_answer_prompt(question, image_paths, memory, detailed=True)
        return self.llm_engine(input_data)

    def generate_direct_output(self, question: str, image_paths: Optional[Union[str, Sequence[str]]], memory: Memory) -> str:
        input_data = self._build_answer_prompt(question, image_paths, memory, detailed=False)
        return self.llm_engine(input_data)
