...
class OpenAIController(BaseLLMController):
    """
    OpenAI controller that routes through this project's engine implementation.

    Why:
    - The project uses Proxy_API_BASE for OpenAI-compatible proxies.
    - engine/openai.py already encapsulates base_url + retry + multimodal handling.
    """

    def __init__(self, model: str = "gpt-4o-mini", api_key: Optional[str] = None, api_base: Optional[str] = None):
        self.model = model
        # Prefer env-based settings used across the project.
        if api_key is None:
            api_key = os.getenv("OPENAI_API_KEY")
        if api_base is None:
            api_base = os.getenv("Proxy_API_BASE") or os.getenv("OPENAI_API_BASE") or os.getenv("OPENAI_BASE_URL")

        # Import the project's engine ChatOpenAI (preferred). Fallback to OpenAI SDK if not available.
        self._engine = None
        try:
            # Normal package path inside agentflow package
            from ..engine.openai import ChatOpenAI as _ChatOpenAI
            # ChatOpenAI reads Proxy_API_BASE itself; keep kwargs minimal.
            self._engine = _ChatOpenAI(
            ...
            # Fallback: direct OpenAI SDK with proxy base_url support (still works with Proxy_API_BASE)
            try:
                from openai import OpenAI
            except ImportError as e:
                raise ImportError(
                    "OpenAI engine not available. Install openai or ensure project engine is importable.") from e
            if api_key is None:
    ...
    def get_completion(self, prompt: str, response_format: dict, temperature: float = 0.7) -> str:
        """
        Return a string (JSON text). A-Mem will json.loads() it downstream.

        NOTE:
        - We intentionally do not use structured output parsing here to keep compatibility
          with both OpenAI SDK and the project's engine wrapper.
        - `response_format` is kept for API compatibility with original A-Mem code.
        """
        if self._engine is not None:
            out = self._engine.generate(
                ...
                max_tokens=1000,
                response_format=None,  # keep engine compatible
            )
            if isinstance(out, dict) and out.get("error"):
                raise RuntimeError(f"LLM error: {out.get('message')}")
            return out

        # Fallback OpenAI SDK path
...
class HybridRetriever:
    ...
    def add_document(self, document: str) -> bool:
        ...
        # [关键修复] 重新初始化 BM25
        # 因为 rank_bm25 不支持增量更新 (没有 add_document 方法)。
        # 我们必须对整个语料库重新分词，并创建一个新的 BM25Okapi 对象。
        tokenized_corpus = [simple_tokenize(doc) for doc in self.corpus]
        self.bm25 = BM25Okapi(tokenized_corpus)

        # 更新向量 (Embeddings 支持增量更新)
        # 注意：这里假设 model.encode 返回的是 numpy array
        doc_embedding = self.model.encode([document])

        # 堆叠 (Stack) 新向量到现有向量矩阵中
        if self.embeddings is None:
            self.embeddings = doc_embedding
        else:
            # 使用 vstack 将新向量追加到矩阵底部
...
class AgenticMemorySystem:
    """Memory management system with Hybrid (BM25 + Embedding) retrieval"""
    ...
    def add_note(self, content: str, time: str = None, **kwargs) -> str:
        ...
        # [修改 2] 使用 add_document (单数) 来追加，而不是 add_documents (复数)
        # HybridRetriever 的 add_documents 会重置索引，而 add_document 会增量更新 BM25 和 Embedding
        doc_str = "content:" + note.content + " context:" + note.context + " keywords: " + ", ".join(
            note.keywords) + " tags: " + ", ".join(note.tags)
        self.retriever.add_document(doc_str)
    ...
    def consolidate_memories(self):
        ...
        # [修改 3] 重新初始化为 HybridRetriever
        self.retriever = HybridRetriever(model_name)

        # [修改 4] 批量处理：HybridRetriever 的 add_documents 更适合一次性批量构建
        # 相比原先在循环里调 add_documents，这里先收集所有文档，再建立索引，效率更高且逻辑正确
        all_docs = []
        for memory in self.memories.values():
            metadata_text = f"{memory.context} {' '.join(memory.keywords)} {' '.join(memory.tags)}"
            all_docs.append(memory.content + " , " + metadata_text)
    ...
    def find_related_memories(self, query: str, k: int = 5) -> List[MemoryNote]:
        ...
        # [修改 5] 调用 HybridRetriever 的 retrieve 方法 (原先是 search)
        # HybridRetriever.retrieve 返回的是 indices 列表
...
# =============================================================================
# FreeAskAgent integration: facade Memory that keeps the original AgentFlow API
# and exposes A-Mem (AgenticMemorySystem) capabilities without deleting A-Mem code.
# =============================================================================

from typing import Any, Dict, List, Optional, Sequence, Union  # noqa: E402
import os  # noqa: E402


class Memory:
    """AgentFlow memory facade.

    - Backward compatible with the original `Memory` used by Planner/Executor:
        - set_query / get_query
        - add_file / get_files
        - add_action / get_actions
    - Additionally exposes A-Mem:
        - add_note(content, **kwargs)
        - retrieve(query, k=5) -> (memory_str, indices)
        - get_retrieved_context(query, k=5) -> str
    """

    def __init__(
            self,
            enable_amem: bool = True,
            amem_kwargs: Optional[Dict[str, Any]] = None,
    ):
        ...
        self.amem: Optional[AgenticMemorySystem] = None
        if enable_amem:
            amem_kwargs = amem_kwargs or {}
            # Allow env overrides without forcing users to change code.
            if "api_key" not in amem_kwargs:
                amem_kwargs["api_key"] = os.getenv("OPENAI_API_KEY")
            if "api_base" not in amem_kwargs:
                amem_kwargs["api_base"] = os.getenv("OPENAI_BASE_URL") or os.getenv("OPENAI_API_BASE")
            try:
                self.amem = AgenticMemorySystem(**amem_kwargs)
            except Exception as e:
                # Do not hard-fail import/run if optional deps/config are missing.
                print(f"[Memory] Warning: failed to init A-Mem AgenticMemorySystem: {e}")
                self.amem = None

    # -------------------------
    # Original AgentFlow memory
    # -------------------------
    ...
    def add_action(self, step_count: int, tool_name: str, sub_goal: str, command: str, result: Any) -> None:
        ...
        # Optionally also store tool results into A-Mem for later retrieval.
        if self.amem is not None:
            try:
                content = f"tool:{tool_name}\nsub_goal:{sub_goal}\ncommand:{command}\nresult:{result}"
                self.amem.add_note(content=content)
            except Exception as e:
                print(f"[Memory] Warning: failed to add tool result into A-Mem: {e}")
    ...
    def add_note(self, content: str, **kwargs) -> Optional[str]:
        if self.amem is None:
            return None
        return self.amem.add_note(content=content, **kwargs)

    def retrieve(self, query: str, k: int = 5):
        if self.amem is None:
            return "", []
        return self.amem.find_related_memories(query=query, k=k)

    def get_retrieved_context(self, query: str, k: int = 5) -> str:
        memory_str, _ = self.retrieve(query=query, k=k)
        return memory_str
