...
class SolverEmbodied:
    ...
    def solve(self, question: str, image_paths: Optional[Union[str, Sequence[str]]] = None):
        ...
        # Sync into memory (for prompts + A-Mem)
        try:
            self.memory.set_query(question)
            if image_paths:
                self.memory.add_file(image_paths)
...
def construct_solver_embodied(llm_engine_name: str = "gpt-4o",
                              enabled_tools: list[str] = ["all"],
                              tool_engine: list[str] = ["Default"],
                              model_engine: list[str] = ["trainable", "dashscope", "dashscope"],
                              # [planner_main, planner_fixed, executor]
                              output_types: str = "final,direct",
                              max_steps: int = 10,
                              max_time: int = 300,
                              max_tokens: int = 4000,
